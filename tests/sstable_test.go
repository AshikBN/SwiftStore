...
func TestSSTable(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		sstable, err := swiftdb.SerializeToSSTable(memtable.GetEntries(), testFileName)
		assert.Nil(t, err)
		defer sstable.Close()
		...
		if reopenFile {
			...
			sstable, err = swiftdb.OpenSSTable(testFileName)
			assert.Nil(t, err)
		}
...
func TestRangeScan(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		assert.Equal(t, swiftdb.Command_DELETE, entries[1].Command)
		...
		assert.Equal(t, swiftdb.Command_DELETE, entries[3].Command)
...
func TestSSTableIterator(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		// Read all entries from the SSTable.
		entries := make([]*swiftdb.LSMEntry, 0)
		for iter != nil {
...
func populateMemtableWithTestData(memtable *swiftdb.Memtable) {
